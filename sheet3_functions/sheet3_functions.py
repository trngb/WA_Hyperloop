...
#            print "skipping crop with lu-class {0}, not on LU-map".format(crop[4])            
            cropmask_fh=crop[4]
            cropsubclass=int(os.path.split(cropmask_fh)[-1].split('.')[0])
            start_dates, end_dates = import_growing_seasons(crop[0])
            ...
            for subcrop in crop[1]:            
                ...
                result = calc_Y_WP_year(result_seasonly, os.path.join(output_dir, 'WP_Y_Yearly_csvs'), crop[1][subcrop],cropsubclass) 
            ...
            finalresult= summarize_subcrops(WP_Y_Yearly_csvs_list,os.path.join(output_dir, 'WP_Y_Yearly_csvs'),cropsubclass)
            if cropsubclass>50:
